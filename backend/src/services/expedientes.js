...
async function exigirExpediente(usuarioId, id) {
  const [[fila]] = await pool.query("SELECT * FROM expedientes WHERE id = ? AND usuario_id = ?", [id, usuarioId]);
  if (!fila) throw new AppError("NO_ENCONTRADO", "El expediente no existe.", 404);
  return fila;
}
...
export async function listar(usuarioId, estado) {
  const [rows] = await pool.query(
    `SELECT e.*,
       ...
       FROM expedientes e
      WHERE e.usuario_id = :usuarioId AND (:estado IS NULL OR e.estado = :estado)
      ORDER BY e.actualizado_en DESC
      LIMIT 300`,
    { usuarioId, estado: ESTADOS.has(estado) ? estado : null },
  );
  return rows.map(expedienteAVista);
}
...
export async function obtener(usuarioId, id) {
  ...
  const [tareas] = await pool.query("SELECT * FROM tareas WHERE expediente_id = ? ORDER BY estado, orden, id", [id]);
  ...
  return { ...expedienteAVista(fila), partes, tareas: tareas.map(tareaAVista), presupuestos, ejecucion, protocolo };
}
...
export async function actualizar(usuarioId, id, datos) {
  validarDatos(datos);
  await exigirExpediente(usuarioId, id);
  ...
  await pool.query(
    "UPDATE expedientes SET caratula = :caratula, tipo_acto = :tipoActo, observaciones = :observaciones, protocolo_id = :protocoloId WHERE id = :id AND usuario_id = :usuarioId",
    { id, usuarioId, caratula: datos.caratula.trim().slice(0, 200), tipoActo: datos.tipoActo || "otro", observaciones: datos.observaciones?.trim().slice(0, 1000) || null, protocoloId },
  );
...
export async function actualizarEstado(usuarioId, id, estado) {
  ...
  const [r] = await pool.query("UPDATE expedientes SET estado = ? WHERE id = ? AND usuario_id = ?", [estado, id, usuarioId]);
...
/** Reemplaza la lista completa de partes: [{ clienteId, rol }]. */
...
export async function crearTarea(usuarioId, expedienteId, datos) {
  await exigirExpediente(usuarioId, expedienteId);
  validarTarea(datos);
  const [r] = await pool.query(
    "INSERT INTO tareas (usuario_id, expediente_id, descripcion, responsable, vence_en, orden) VALUES (:usuarioId, :expedienteId, :descripcion, :responsable, :venceEn, :orden)",
    { usuarioId, expedienteId, descripcion: datos.descripcion.trim().slice(0, 300), responsable: datos.responsable?.trim().slice(0, 80) || null, venceEn: datos.venceEn || null, orden: await siguienteOrden(expedienteId) },
  );
  const [[t]] = await pool.query("SELECT * FROM tareas WHERE id = ?", [r.insertId]);
...
export async function actualizarTarea(usuarioId, expedienteId, tareaId, datos) {
  validarTarea(datos);
  const [r] = await pool.query(
    "UPDATE tareas SET descripcion = :descripcion, responsable = :responsable, vence_en = :venceEn WHERE id = :tareaId AND expediente_id = :expedienteId AND usuario_id = :usuarioId",
    { tareaId, expedienteId, usuarioId, descripcion: datos.descripcion.trim().slice(0, 300), responsable: datos.responsable?.trim().slice(0, 80) || null, venceEn: datos.venceEn || null },
  );
  if (r.affectedRows === 0) throw new AppError("NO_ENCONTRADO", "La tarea no existe.", 404);
  const [[t]] = await pool.query("SELECT * FROM tareas WHERE id = ?", [tareaId]);
  return tareaAVista(t);
}
...
export async function actualizarEstadoTarea(usuarioId, expedienteId, tareaId, estado) {
  ...
  const [r] = await pool.query(
    "UPDATE tareas SET estado = :estado, completada_en = IF(:estado = 'hecha', NOW(), NULL) WHERE id = :tareaId AND expediente_id = :expedienteId AND usuario_id = :usuarioId",
    { estado, tareaId, expedienteId, usuarioId },
  );
  if (r.affectedRows === 0) throw new AppError("NO_ENCONTRADO", "La tarea no existe.", 404);
  const [[t]] = await pool.query("SELECT * FROM tareas WHERE id = ?", [tareaId]);
  return tareaAVista(t);
...
export async function borrarTarea(usuarioId, expedienteId, tareaId) {
  const [r] = await pool.query("DELETE FROM tareas WHERE id = ? AND expediente_id = ? AND usuario_id = ?", [tareaId, expedienteId, usuarioId]);
  if (r.affectedRows === 0) throw new AppError("NO_ENCONTRADO", "La tarea no existe.", 404);
}
...
export async function crearTareasDesdeSesion(usuarioId, expedienteId, sesion) {
  ...
  if (sesion.ejecucionId) await pool.query("UPDATE expedientes SET ejecucion_id = ? WHERE id = ? AND usuario_id = ?", [sesion.ejecucionId, expedienteId, usuarioId]);
