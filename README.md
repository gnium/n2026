...
- **Integraciones con Google** (pantalla *Integraciones*): agenda en Google Calendar y copia de documentos en Drive. El correo se maneja fuera de la app a propósito: `gmail.send` es un alcance *restringido* de Google y obliga a una auditoría de seguridad anual de un tercero para publicar la aplicación. **Viene apagada** y se enciende en dos pasos:
  1. **Se configura el proyecto una sola vez por instalación** (lo hace quien opera la app, no cada escribano): en [console.cloud.google.com](https://console.cloud.google.com) crear un proyecto, activar las APIs de *Google Calendar* y *Google Drive*, completar la pantalla de consentimiento y crear credenciales de tipo **ID de cliente de OAuth · Aplicación web**. En *URI de redirección autorizados* pegar exactamente `<APP_URL>/api/google/callback`. Las credenciales se cargan de dos maneras: como `GOOGLE_CLIENT_ID` y `GOOGLE_CLIENT_SECRET` en el `.env` del servidor (queda fijo en el despliegue), o desde Integraciones → *Credenciales de Google Cloud* (se guardan cifradas y tienen prioridad sobre el `.env`).
     - Si la escribanía tiene **Google Workspace**, elegir tipo de usuario **Interno**: no requiere verificación de Google y el permiso no vence. Con una cuenta personal solo está disponible **Externo**, y en modo *prueba* Google vence el permiso a los 7 días (la app lo detecta y pide reconectar).
  2. **Cada cuenta conecta su Google** con *Conectar con Google*. Los permisos que pide son los mínimos: `calendar.events` (crear y editar eventos, no leer el calendario) y `drive.file` (solo los archivos que crea la app). **No pide ningún permiso sobre el correo.**
